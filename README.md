...
| 检测目标 | 钢珠（直径约 1cm），在 PPR 水管凹槽内 |
| 输出 | 钢珠坐标 (X, Y)，单位 cm，精度 0.1cm |
| 帧率 | **33fps 平均**（含曝光 + 推理，持续最大帧率模式） |
| 接口 | UART 115200bps，帧格式见 2.8 节 |
| 模型 | 轻量检测模型（yolo-n 等），ROI 裁切缩小搜索区域 |
...
| 模块 | 文件 | 职责 |
|------|------|------|
| K230 串口驱动 | `bsp/bsp_uart2.c/h` | K230 专用 DMA 串口接收 |
| K230 协议解析 | `middleware/mid_k230.c/h` | 帧解析（`X:+03.5,Y:-01.2\r\n`），钢球坐标提取 |
| 钢球闭环控制 | `app/app_ball_control.c/h` | 位置 PID + 前馈 + 定点状态机 |
...
| 术语 | 英文 | 舵机角度变化 | 齿条运动 | 摆杆右端 |
|------|------|-------------|---------|---------|
| 正转 | CW (Clockwise) | **增大**（135° → 270° 方向） | 齿条**下降** | 下沉 → 凹槽向右倾斜 → 球向 + 方向 |
| 逆转 | CCW (Counter-Clockwise) | **减小**（135° → 0° 方向） | 齿条**上升** | 抬高 → 凹槽向左倾斜 → 球向 - 方向 |

> 正转定义为齿条下降、小球向正方向滚动（符合赛题要求）。PWM 脉冲宽度与角度成**正比**（500µs = 0°, 2500µs = 270°），正转（CW）对应脉冲增大、逆转（CCW）对应脉冲减小。
...
> **符号约定**：管道倾角 φ 以**上倾为正**（右端抬高 → 球向 - 方向滚）、**下倾为负**（右端下沉 → 球向 + 方向滚）。

```
φ(deg) = -0.0640 × Δθ_servo          Δθ_servo = servo offset from 135° (CW = +)
a_ball  = -0.0110 × Δθ_servo  [m/s²]  (ignore friction, small-angle approx)
PWM     = 1500 - 116 × φ_des(deg)     (direct skip-hard-calculation formula)
```

| Δθ_servo | φ (deg) | a_ball (m/s²) | 说明 |
|-----------|---------|---------------|------|
| +30° (CW, 正转) | **-1.92°** | +0.328 | 大角度加速，球→+cm |
| +25° (CW, 正转) | **-1.60°** | +0.274 | 制动/加速 |
| +20° (CW, 正转) | **-1.28°** | +0.219 | |
| +10° (CW) | **-0.64°** | +0.110 | |
| +5° (CW) | **-0.32°** | +0.055 | |
| 0° (中位) | **0°** | 0 | 水平 |
| -5° (CCW) | **+0.32°** | -0.055 | |
| -20° (CCW, 逆转) | **+1.28°** | -0.219 | 回滚加速，球→-cm |
| -25° (CCW, 逆转) | **+1.60°** | -0.274 | 制动/加速 |
...
| 静摩擦死区 φ_min | 0.1°~0.3° (预估) | **待测试** | 球置中心 O，逐步增加倾角 |
...
开环时序脉冲控制（无 K230 传感器反馈），大角度使重力主导摩擦：
...
Phase 5: SETTLE   回中位 → 静态摩擦稳住                500ms
    │
    ▼
  DONE (总耗时约 2.55s)
```
...
> 与 README 文档定义相反，舵机实际安装方向：

| offset 符号 | 舵机运动 | 齿条 | 球运动 |
|------------|---------|------|--------|
| `-` (负) | CCW 逆转 | 上升 | 球 → **+cm** |
| `+` (正) | CW 正转 | 下降 | 球 → **-cm** |
...
| `SETTLE_ANGLE` | 0° | Phase 5，回中位 |
| `SETTLE_TICKS` | 50 | 500ms |
| **总耗时** | **2.55s** | 远小于 5s 限制 |

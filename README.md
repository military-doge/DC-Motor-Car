...
|------|------|-------------|---------|---------|
| 正转 | CCW (Counter-Clockwise) | **减小**（135° → 0° 方向） | 齿条**上升** | 抬高 → 凹槽向左倾斜 |
| 反转 | CW (Clockwise) | **增大**（135° → 270° 方向） | 齿条**下降** | 下沉 → 凹槽向右倾斜 |

> 逆时针为正转，因为能使齿条向上运动。PWM 脉冲宽度与角度成**正比**（500µs = 0°, 2500µs = 270°），正转（CCW）对应脉冲减小、反转（CW）对应脉冲增大。
...
| 舵机信号引脚 | — | PA0 | 参见引脚分配 |
| 铰链到齿条支撑点距离 | L | **224 mm（实测）** | 卡尺实测，水管水平时 |
...
**机械减速比** = L / 15.625。以 L = 224mm 为例，减速比 **14.3:1**（舵机转 14.3° 摆杆偏 1°）。

| θ_servo (@ L=224mm) | h | φ | 说明 |
|---------------------|------|------|------|
| 0.54° (舵机误差) | 0.147 mm | **0.038°** | 舵机误差经减速后仅 0.038° |
| 1° | 0.273 mm | **0.070°** | |
| 5° | 1.36 mm | **0.35°** | |
| 10° | 2.73 mm | **0.70°** | |
...
|---------|-------------------|------|
| 0.54° (误差) | 0.0064 m/s² | 舵机误差导致的加速度扰动，可忽略 |
| 1° | 0.0119 m/s² | |
| 5° | 0.0596 m/s² | |
| 8.4° | **0.100 m/s²** | 有效回复加速度 |
| 10° | 0.119 m/s² | |

#### 关键结论

1. **大减速比容忍舵机误差**：L=224mm 时减速比 14.3:1，舵机 0.54° 误差仅导致摆杆倾角 0.038°、加速度扰动 0.0064 m/s²，完全可接受。

2. **舵机工作范围远小于满量程**：产生 0.1 m/s² 回复加速度仅需舵机偏转约 8.4°（摆杆倾角约 0.59°）。舵机绝大部分时间在中位 135°±8.4° 内工作，仅用了满量程 270° 的约 6%。

3. **弯道离心力是最大扰动源**：v=0.42m/s, R=0.5m 时离心加速度 0.353 m/s²，需补偿舵机偏转约 29.6°，仍在可用范围内。

4. **二级公式（直接跳过硬计算）**：

```
PWM = 1500 + φ_des(deg) × 106.2
    = 1500 + φ_des(rad) × 224 × 27.2
```

> 推导：PWM = 1500 + (φ × L / 15.625) × 2000/270 = 1500 + φ × 224 × (2000 / 4219) = 1500 + φ × 106.2
...
| 项目 | 当前值 | 实际值 | 备注 |
|------|--------|--------|------|
| 轴距 L (摆杆铰链→齿条支撑点) | 224mm | **已实测** | 卡尺实测，水管水平时 |
| 轮距 (track) | 0.21m (210mm) | **待实测** | 影响转弯半径计算 |
| 齿轮齿条线性度 | — | **待标定** | PWM 500→2500μs 步长 100μs |
...
- `APP_Test_Init()` — 只设置 PWM 比较值，timer 启停由 `BSP_Servo_Init()` 管理
- `APP_Test_TimerTick()` — 10ms ISR 内跑位置式 PD 控制器
- `APP_Test_Run/Start/Stop/IsRunning` — 空壳，未实现
- PID 参数: KP=6.0, KD=3.0 (位置式，未经调参)
